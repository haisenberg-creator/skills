...
Write the patch to a temporary file (such as `<session>/patch.json`), then apply it:

```sh
node $SKILL/server.mjs patch --session <session> --file <session>/patch.json
```

(`--file <path>` reads the patch from a file; piping on stdin or heredocs are also supported where available.) It merges the patch,
  ...
  "questions": [
    ...
    },
    {
      ...
      "thread": [
        {
          "who": "user",
          "text": "Why not keep staged answers in localStorage?",
          "at": "2026-09-18T21:39:58Z"
        },
        {
          "who": "agent",
          "text": "localStorage is per browser profile, so a second browser or a cleared profile loses them. The session folder survives both, at the cost of one more file."
        }
      ]
    },
    {
      ...
      "round": 4,
      "deps": ["q2"],
      "title": "Who owns the retry budget?",
      "body": "With the server-side queue settled in Q2, retries need an owner.",
      "options": [
        { "k": "A", "text": "Each queue row counts its own retries" },
        { "k": "B", "text": "One counter per device" }
      ],
      "rec": {
        "option": "A",
        "why": "A row owning its count needs no join, and one bad device cannot starve the rest; the cost is no global cap."
      }
    }
  ]
}
...
## Start (`/grill-with-ui <topic>`, `/grill-with-ui <topic> --with-docs`, `$grill-with-ui <topic>`, or "grill with ui: <topic>")
...
3. Read `<session>/state.json` once to load the grill (reading is fine; only writes go
   ...
   is the last seq.
4. Continue with Start steps 3 to 5. `serve` retries the port it used last time, so a tab the user
...
6. Send everything from steps 2 to 5 **in ONE patch**, together with
...
4. If `--with-docs` was passed at start:
   In addition to writing the design doc, also write settled durable decisions (`durable: true`) to `docs/adr/` (following the project's ADR template if one exists) and update `CONTEXT.md` with newly settled terms from `terms`.
...
6. Print one line with the doc path (and the visual's). End.
...
2. Run `node $SKILL/server.mjs wait --session <session> --after <agent.handled>`.
   ...
   `--timeout` (default 480) bounds how long that wait _process_ sits idle before exiting 3;
  ...
  "questions": [
    {
      ...
      "status": "open|answered|deferred|reopened",
      "durable": false,
      "updated": false,
      "answer": { "kind": "accept|option|text", "option": "A", "text": "…" },
      "explore": {
        "at": "ISO",
        "rows": [{ "option": "A", "pros": ["…"], "cons": ["…"] }],
      }, // after an explore action
      "thread": [{ "who": "user|agent", "text": "…", "at": "ISO" }],
    },
  ],
}
...
```jsonc
{
  ...
  "seq": 12,
  "at": "ISO",
  "session": "/abs/session/folder",
  "actions": [
    {
      "q": "q15",
      "type": "answer",
      "kind": "accept|option|text",
      "option": "A",
      "text": "…",
    },
