...
const HOME =
  process.env.GRILL_HOME || path.join(os.homedir(), ".grill-with-ui");
...
const die = (msg, code = 2) => {
  process.stderr.write(`grill: ${msg}\n`);
  process.exit(code);
};
...
function cmdNew(o) {
  ...
  for (let n = 2; fs.existsSync(session); n++)
    session = path.join(dir, `${id}-${n}`);
  ...
  writeJson(path.join(session, "state.json"), {
    ...
    project,
    created: now,
    agent: { status: "working", since: now },
    terms: [],
    questions: [],
  });
...
function readState(session) {
  try {
    return JSON.parse(
      fs.readFileSync(path.join(session, "state.json"), "utf8")
    );
  } catch {
    return null;
  }
  ...
  const srv = http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && pathname === "/events")
      return send(
        res,
        200,
        fs.existsSync(events) ? fs.readFileSync(events) : "",
...
function cmdWait(o) {
  ...
  const after =
    o.after !== undefined && o.after !== true
      ? Number(o.after)
      : lastSeq(events);
  const deadline =
    Date.now() +
    Number(o.timeout !== undefined && o.timeout !== true ? o.timeout : 480) *
      1000;
  const tick = () => {
...
function cmdUrl(o) {
  ...
  const deadline =
    Date.now() +
    Number(o.timeout !== undefined && o.timeout !== true ? o.timeout : 5) *
      1000;
  const tick = () => {
    try {
...
const fieldsOf = (p, where) => {
  if (Object.hasOwn(p, "__proto__"))
    bad(`"__proto__" in ${where} is not a state.json field`);
...
function stampTimes(p, state, now) {
  ...
  const messages = (list) =>
    Array.isArray(list) ? list.map((m) => fill(m, "at")) : list;
  const out = { ...p };
  if (isObj(out.agent) && out.agent.status != null)
    out.agent = fill(out.agent, "since");
  ...
  if (isObj(out.visual)) {
    let v = { ...out.visual };
    const current =
      isObj(state) && isObj(state.visual) ? state.visual.version : undefined;
...
function appendTo(current, added, where) {
  if (!Array.isArray(added))
    bad(`${where} in a patch must be an array of the new entries to append`);
  if (current !== undefined && !Array.isArray(current))
    bad(`${where} in state.json is not an array`);
...
function patchQuestions(current, list) {
  if (!Array.isArray(list))
    bad("questions in a patch must be an array of entries, each with an id");
  ...
  for (const p of list) {
    if (!isObj(p) || typeof p.id !== "string" || !p.id)
      bad("every question entry in a patch needs a string id");
    const i = qs.findIndex((q) => isObj(q) && q.id === p.id);
    if (i >= 0) {
      qs[i] = mergeOne(qs[i], p, p.id, THREAD);
      continue;
    }
    if (p.title == null) {
      const ids = qs.map((q) => q && q.id).join(", ") || "none yet";
      bad(
        `no question ${JSON.stringify(p.id)} in state.json (ids: ${ids}); a new question needs round, title, and rec`
      );
    }
    const missing = ["round", "rec"].filter((k) => p[k] == null);
    if (missing.length)
      bad(`new question ${p.id} needs ${missing.join(" and ")}`);
    const q = mergeOne({}, p, p.id, THREAD);
    for (const [k, d] of Object.entries(newQuestionDefaults()))
      if (!(k in q)) q[k] = d;
    qs.push(q);
  }
  return qs;
...
function patchTerms(current, list) {
  if (!Array.isArray(list))
    bad("terms in a patch must be an array of entries, each with a term");
  if (current != null && !Array.isArray(current))
    bad("terms in state.json is not an array");
  const ts = (current || []).slice();
  for (const t of list) {
    if (!isObj(t) || typeof t.term !== "string" || !t.term)
      bad("every term entry in a patch needs a string term");
...
function applyPatch(state, p, now) {
  ...
  for (const [k, v] of fieldsOf(stampTimes(p, state, now), "the patch")) {
    if (v === null) delete out[k];
      ...
      out.visual = mergeOne(out.visual, v, "visual", VISUAL_APPENDS);
    else if (k === "questions")
      out.questions = patchQuestions(out.questions, v);
...
function validateState(s) {
  ...
  const check = (o, k, ok, msg) => {
    if (k in o) need(ok(o[k]), msg);
  };
  const texts = (o, keys, where) => {
    for (const k of keys) check(o, k, str, `${where}.${k} must be a string`);
  };
  const messages = (list, where) => {
    ...
    list.forEach((m, i) =>
      need(
        isObj(m) &&
          (m.who === "user" || m.who === "agent") &&
          str(m.text) &&
          (m.at === undefined || str(m.at)),
        `${where}[${i}] must be {"who":"user"|"agent","text":"…","at":"ISO"}`
      )
    );
  };
  for (const k of ["topic", "doc", "project", "created", "note"])
    check(s, k, str, `${k} must be a string`);
  if ("finished" in s) {
    need(
      isObj(s.finished),
      'finished must be an object ({"doc","visual","at"})'
    );
  ...
  if ("agent" in s) {
    ...
    check(
      s.agent,
      "status",
      (v) => v === "waiting" || v === "working",
      'agent.status must be "waiting" or "working"'
    );
    ...
    check(
      ...
      "handled",
      count,
      "agent.handled must be a whole number (the seq of the last handled send)"
    );
  }
  if ("terms" in s) {
  ...
  s.questions.forEach((q, i) => {
    need(
      isObj(q) && str(q.id) && q.id !== "",
      `questions[${i}] needs a string id`
    );
    const w = q.id;
    need(!ids.has(w), `question id ${w} appears twice`);
    ids.add(w);
    need(Number.isFinite(q.round), `${w}.round must be a number`);
    need(str(q.title), `${w}.title must be a string`);
    need(
      STATUSES.includes(q.status),
      `${w}.status must be one of ${STATUSES.join("|")}`
    );
    need(
      isObj(q.rec),
      `${w}.rec must be an object ({"option","why"} or {"text","why"})`
    );
    ...
    check(
      ...
      (v) =>
        Array.isArray(v) &&
        v.every((x) => isObj(x) && str(x.k) && (!("text" in x) || str(x.text))),
      `${w}.options must be an array of {"k","text"} with string k and text`
    );
    if ("answer" in q) {
      need(
        isObj(q.answer) && ANSWER_KINDS.includes(q.answer.kind),
        `${w}.answer.kind must be one of ${ANSWER_KINDS.join("|")}`
      );
    ...
    if ("explore" in q) {
      const e = q.explore;
      need(
        isObj(e) && Array.isArray(e.rows),
        `${w}.explore must be {"at","rows":[…]}`
      );
      ...
      e.rows.forEach((r, i) => {
        ...
        for (const k of ["pros", "cons"])
          check(r, k, strs, `${where}.${k} must be an array of strings`);
      });
    }
    for (const k of ["durable", "updated"])
      check(q, k, bool, `${w}.${k} must be true or false`);
  ...
  if ("visual" in s) {
    ...
    check(
      v,
      "kind",
      (x) => x === "prototype" || x === "diagram",
      'visual.kind must be "prototype" or "diagram"'
    );
    check(v, "version", count, "visual.version must be a whole number");
    need(
      "kind" in v && "version" in v,
      "visual needs kind and version; the first Visualize draw creates it (version 0)"
    );
    ...
    check(
      v,
      "drawing",
      (x) => isObj(x) && (!("since" in x) || str(x.since)),
      'visual.drawing must be {"since":"ISO","seq":N}'
    );
...
function cmdPatch(o) {
  ...
  if (o.file !== undefined) {
    ...
    try {
      ...
      die(
        `cannot read the patch file ${o.file}: ${e.code || oneLine(e.message)}`
      );
    }
  } else {
    ...
    if (tty.isatty(0))
      die("no patch: pipe a JSON patch on stdin or pass --file <path>");
    try {
      text = fs.readFileSync(0, "utf8");
    } catch (e) {
      die(`cannot read the patch from stdin: ${e.code || oneLine(e.message)}`);
    }
  }
  if (!text.trim())
    die("empty patch: send a JSON object shaped like state.json");
  let p;
  try {
    p = JSON.parse(text);
  } catch (e) {
    die(
      `the patch is not valid JSON (state.json unchanged): ${oneLine(e.message)}`
    );
  }
  if (!fs.existsSync(file))
    die(`no state.json in ${session}; \`new\` creates it`);
  let state;
  try {
    state = JSON.parse(fs.readFileSync(file, "utf8"));
  } catch (e) {
    die(
      `state.json in ${session} is not valid JSON, so it was not patched: ${oneLine(e.message)}`
    );
  }
  let next;
  try {
...
(Object.hasOwn(cmds, o._[0] ?? "")
  ? cmds[o._[0]]
  : () =>
      die(
        "usage: server.mjs new|serve|sessions|pending|wait|url|patch [--session DIR] ..."
      ))(o);
