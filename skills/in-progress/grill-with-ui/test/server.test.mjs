...
function lineReader(stream) {
  ...
  const nth = (n) =>
    new Promise((res) => {
      const check = () =>
        lines.length >= n ? res(lines[n - 1]) : waiters.push(check);
      check();
    });
  return { lines, nth };
}
async function startServe(session, extra = []) {
  const child = spawn(
    process.execPath,
    [SERVER, "serve", "--session", session, ...extra],
...
const newSession = (cwd, topic = "Fonts") =>
  JSON.parse(run(["new", "--topic", topic], { cwd }));
...
test("new: outside git the key comes from the cwd; state.json skeleton is written", () => {
  ...
  assert.ok(
    out.session.startsWith(join(home, "sessions", out.key)),
    out.session
  );
  const state = JSON.parse(
    readFileSync(join(out.session, "state.json"), "utf8")
  );
...
test("serve: ready line + server.json, page, state, send appends the same line it prints, bad bodies are 400, seq survives restart", async (t) => {
  ...
  assert.equal(
    JSON.parse(readFileSync(join(session, "server.json"), "utf8")).url,
    s.ready.url
  );
  ...
  const actions = [
    { q: "q1", type: "answer", kind: "text", text: "B, bundle them" },
  ];
  ...
  const fileLines = readFileSync(join(session, "events.jsonl"), "utf8")
    .trim()
    .split("\n");
  ...
  assert.equal(
    readFileSync(join(session, "events.jsonl"), "utf8").trim().split("\n")
      .length,
    1
  );
  ...
  t.after(s2.stop);
  const r2 = await post(s2.ready.url, {
    actions: [{ q: "q1", type: "thread", text: "why not C?" }],
  });
...
test("serve: /send rejects a mismatched Origin, allows same-origin and no-Origin requests", async (t) => {
  ...
  assert.equal(
    (await post(s.ready.url, { actions })).status,
    200,
    "no Origin header (curl, wait mode, tests) is allowed"
  );
  assert.equal(
    (await withOrigin(s.ready.url.slice(0, -1))).status,
    200,
    "the page's own origin is allowed"
  );
  const port = Number(new URL(s.ready.url).port);
  assert.equal(
    (await withOrigin(`http://localhost:${port}`)).status,
    200,
    "the same server opened as localhost is allowed"
  );
  assert.equal(
    (await withOrigin(`http://localhost:${port + 1}`)).status,
    403,
    "localhost on another port is still rejected"
  );
  assert.equal(
    (await withOrigin("https://evil.example", "text/plain")).status,
    403,
    "a foreign origin is rejected even as a no-preflight content-type"
  );
  assert.equal(
    (await withOrigin("null")).status,
    403,
    "the sandboxed visual iframe's opaque origin is rejected too"
  );
  ...
  assert.equal(
    readFileSync(join(session, "events.jsonl"), "utf8").trim().split("\n")
      .length,
    3,
    "only the three accepted sends landed"
  );
});
...
test("wait: blocks for a seq newer than --after (default: current last), prints it, exits 0; exit 3 on timeout", async (t) => {
  ...
  const w = spawn(
    process.execPath,
    [SERVER, "wait", "--session", session, "--after", "1", "--timeout", "5"],
    { env, stdio: ["ignore", "pipe", "inherit"] }
  );
  const wo = lineReader(w.stdout);
  await sleep(400);
  assert.equal(wo.lines.length, 0, "nothing printed before a new send");
  await post(s.ready.url, {
    actions: [{ q: "q1", type: "answer", kind: "option", option: "A" }],
  });
  ...
  const w2 = spawn(
    process.execPath,
    [SERVER, "wait", "--session", session, "--timeout", "20"],
    { env, stdio: ["ignore", "pipe", "inherit"] }
  );
  ...
  assert.ok(
    seen2.seq > 2,
    `default --after skips everything already on disk (printed seq ${seen2.seq})`
  );
  assert.equal(
    seen2.actions[0].type,
    "reopen",
    "and prints a send that arrived after it started"
  );

  const w3 = spawn(
    process.execPath,
    [SERVER, "wait", "--session", session, "--timeout", "0.5"],
    { env, stdio: ["ignore", "pipe", "inherit"] }
  );
...
test("serve: reuses the last port from server.json, falls back to ephemeral when it is taken; --port wins", async (t) => {
  ...
  assert.equal(
    JSON.parse(readFileSync(join(session, "server.json"), "utf8")).port,
    port
  );
  await s1.stop();
  const s2 = await startServe(session);
  ...
  t.after(s3.stop);
  assert.notEqual(
    new URL(s3.ready.url).port,
    String(port),
    "falls back when the port is taken"
  );
  ...
  t.after(s4.stop);
  assert.notEqual(
    new URL(s4.ready.url).port,
    String(port),
    "explicit --port 0 skips reuse"
  );
});
...
test("sessions: lists this project's sessions newest first, unfinished by default, --all includes finished", async () => {
  ...
  sb.questions = [
    { id: "q1", round: 1, status: "answered" },
    { id: "q2", round: 1, status: "open" },
    { id: "q3", round: 2, status: "deferred" },
    { id: "q4", round: 2, status: "reopened" },
  ];
  write(b.session, sb);
  writeFileSync(
    join(b.session, "events.jsonl"),
    '{"seq":1}\n{"seq":2}\n{"seq":3}\n'
  );
  const sc = stateOf(c.session);
  sc.finished = { doc: "docs/x.md", at: "y" };
  write(c.session, sc);

  const lines = run(["sessions"], { cwd })
    .split("\n")
    .map((l) => JSON.parse(l));
  assert.deepEqual(
    lines.map((l) => l.session),
    [b.session, a.session]
  );
  ...
  assert.deepEqual(lines[1], {
    ...
    id: a.id,
    topic: "First topic",
    created: stateOf(a.session).created,
    finished: null,
    open: 0,
    answered: 0,
    handled: 0,
  ...
  const all = run(["sessions", "--all"], { cwd })
    .split("\n")
    .map((l) => JSON.parse(l));
  assert.deepEqual(
    all.map((l) => l.session),
    [c.session, b.session, a.session]
  );
...
test("pending: prints the events past agent.handled, nothing when caught up", async (t) => {
  ...
  for (const k of ["A", "B", "C"])
    await post(s.ready.url, {
      actions: [{ q: "q1", type: "answer", kind: "option", option: k }],
    });
  await s.out.nth(4);
  ...
  delete st.agent.handled;
  writeFileSync(join(session, "state.json"), JSON.stringify(st));
  assert.equal(
    run(["pending", "--session", session]).split("\n").length,
    3,
    "no handled means everything is pending"
  );
});
...
const stateOf = (session) =>
  JSON.parse(readFileSync(join(session, "state.json"), "utf8"));
const rawState = (session) => readFileSync(join(session, "state.json"), "utf8");
const leftovers = (session) =>
  readdirSync(session).filter(
    (f) => !["state.json", "events.jsonl", "server.json"].includes(f)
  );
const qn = (id, round, extra = {}) => ({
  ...
  rec: { option: "A", why: "Alpha is simpler." },
  status: "open",
  durable: false,
  updated: false,
  thread: [],
...
function seeded(fields = {}) {
  ...
  const st = {
    ...stateOf(session),
    agent: { status: "waiting", since: T0, handled: 0 },
    ...fields,
  };
...
function patch(session, body, extra = []) {
  ...
  const r = spawnSync(
    process.execPath,
    [SERVER, "patch", "--session", session, ...extra],
...
function stampedBetween(at, t1, t2, what) {
  ...
  assert.ok(
    t >= t1 - 1000 && t <= t2 + 1000,
    `${what} (${at}) was stamped by this patch`
  );
}
function rejected(session, body, pattern, extra) {
  ...
  assert.notEqual(
    r.code,
    0,
    `expected a rejection for ${typeof body === "string" ? body : JSON.stringify(body)}`
  );
...
test("patch: agent and visual merge one level; other top-level keys are replaced whole; stdout is one short line", () => {
  const session = seeded({
    note: "old note",
    doc: "docs/a-design.md",
    agent: { status: "working", since: T0, handled: 3 },
    questions: [
      qn("q1", 1),
      qn("q2", 1, { status: "answered", answer: { kind: "accept" } }),
    ],
    visual: {
      kind: "prototype",
      version: 2,
      ...
      note: "v2: first",
      stale: true,
      drawing: { since: T0, seq: 3 },
      thread: [{ who: "user", text: "bigger", at: T0 }],
    },
  });
  const r = applied(session, {
    ...
    visual: {
      stale: false,
      note: "v3: bigger",
      drawing: { since: "2026-09-01T11:00:00.000Z", seq: 4 },
    },
    note: "new note",
    doc: "docs/b-design.md",
    finished: { doc: "docs/b-design.md", visual: "docs/b-visual.html", at: T0 },
  });
  const st = stateOf(session);
  assert.deepEqual(st.agent, { status: "working", since: T0, handled: 4 });
  assert.deepEqual(st.visual, {
    kind: "prototype",
    version: 2,
    at: T0,
    note: "v3: bigger",
  ...
  assert.deepEqual(JSON.parse(r.out), {
    ...
    open: 1,
    handled: 4,
    bytes: statSync(join(session, "state.json")).size,
  });
  for (const leak of ["Patch topic", "Title q1", "bigger", "new note"])
    assert.ok(!r.out.includes(leak), `stdout never echoes the state (${leak})`);
  assert.deepEqual(leftovers(session), [], "no temp file left behind");

  applied(session, { finished: { doc: "docs/c-design.md", at: T0 } });
  assert.deepEqual(
    stateOf(session).finished,
    { doc: "docs/c-design.md", at: T0 },
    "finished is replaced whole, not merged"
  );
});
...
test("patch: questions merge one level by id; each given field replaces that field whole", () => {
  ...
  const session = seeded({
    questions: [
      qn("q1", 1, {
        ...
        explore,
        thread: [
          { who: "user", text: "hm", at: T0 },
          { who: "agent", text: "ok", at: T0 },
        ],
      }),
      qn("q2", 1),
    ],
  ...
  assert.deepEqual(
    ...
    ["q1", "q2"],
    "order kept, nothing appended"
  );
  const q1 = st.questions[0];
  assert.deepEqual(
    q1.rec,
    { option: "B" },
    "rec replaced whole, not deep-merged"
  );
  ...
  assert.deepEqual(
    q1.thread,
    before.questions[0].thread,
    "thread untouched when the patch has none"
  );
  assert.deepEqual(
    st.questions[1],
    before.questions[1],
...
test("patch: a new id with a title is appended with defaults; an unknown id without a title is rejected, state untouched", () => {
  ...
  assert.deepEqual(st.questions[1], {
    id: "q2",
    round: 2,
    deps: ["q1"],
    title: "Second",
  ...
  assert.deepEqual(st.questions[2], {
    id: "q3",
    round: 2,
    title: "Free text",
    body: "B3",
  ...
  rejected(
    ...
    {
      agent: { handled: 1 },
      questions: [{ id: "Q1", status: "answered", answer: { kind: "accept" } }],
    },
    /Q1/
  );
  rejected(
    session,
    { questions: [{ id: "q9", round: 3, title: "No rec" }] },
    /q9.*rec/
  );
  rejected(
    session,
    { questions: [{ id: "q9", title: "No round", rec: { option: "A" } }] },
    /q9.*round/
  );
...
test("patch: thread and visual.queued append; appended messages without at get the current time", () => {
  ...
  const session = seeded({
    ...
    visual: {
      ...
      version: 1,
      at: T0,
      thread: [m("agent", "v1 drawn")],
      drawing: { since: T0, seq: 1 },
      queued: ["feedback: bigger"],
    },
  });
  const t1 = Date.now();
  applied(session, {
    questions: [
      {
        id: "q1",
        thread: [
          m("user", "why?", "2026-09-01T10:05:00.000Z"),
      ...
      },
      {
        ...
        title: "New",
        rec: { text: "t", why: "w" },
        thread: [{ who: "agent", text: "context" }],
      },
    ],
    visual: {
      thread: [{ who: "user", text: "smaller" }],
      queued: ["feedback: smaller"],
    },
  });
  const t2 = Date.now();
  const st = stateOf(session);
  const within = (at) => {
    assert.match(at, ISO);
    const t = Date.parse(at);
    assert.ok(t >= t1 - 1000 && t <= t2 + 1000, at);
  };
  const th = st.questions[0].thread;
  assert.deepEqual(
    th.slice(0, 2),
    [m("user", "first"), m("user", "why?", "2026-09-01T10:05:00.000Z")],
    "existing kept, given at kept"
  );
  assert.equal(th.length, 3);
  assert.equal(th[2].text, "because");
  within(th[2].at);
  within(st.questions[1].thread[0].at);
  assert.deepEqual(st.visual.thread[0], m("agent", "v1 drawn"));
  assert.equal(st.visual.thread[1].text, "smaller");
  within(st.visual.thread[1].at);
  assert.deepEqual(st.visual.queued, ["feedback: bigger", "feedback: smaller"]);
  assert.deepEqual(
    st.visual.drawing,
    { since: T0, seq: 1 },
    "the rest of visual is kept"
  );
  rejected(
    session,
    { questions: [{ id: "q1", thread: { who: "user", text: "not a list" } }] },
    /thread/
  );
...
test("patch: the server stamps the times the agent leaves out; an explicit time in the patch always wins", () => {
  ...
  stampedBetween(
    st.questions[2].explore.at,
    t1,
    t2,
    "q3.explore.at (new question)"
  );
  ...
  applied(session, {
    ...
    finished: {
      doc: "docs/x-design.md",
      visual: "docs/x-visual.html",
      at: at(5),
    },
  });
  st = stateOf(session);
  ...
  applied(session, {
    agent: { handled: 5 },
    visual: { version: 3, stale: true },
    questions: [{ id: "q2", status: "deferred" }],
  });
  st = stateOf(session);
  assert.equal(
    st.agent.since,
    before.agent.since,
    "agent.since kept when the patch gives no status"
  );
  assert.equal(
    st.visual.at,
    before.visual.at,
    "visual.at kept when the version does not change"
  );
  assert.deepEqual(
    st.visual.drawing,
    before.visual.drawing,
    "drawing untouched"
  );
  assert.deepEqual(
    st.questions[1].explore,
    before.questions[1].explore,
    "explore untouched"
  );
  ...
  t2 = Date.now();
  stampedBetween(
    stateOf(session).finished.at,
    t1,
    t2,
    "finished.at when finished is re-given"
  );
});
...
test("patch: null deletes a key at any level", () => {
  const session = seeded({
    ...
    questions: [
      qn("q1", 1, { status: "answered", answer: { kind: "accept" } }),
      qn("q2", 1),
    ],
    visual: {
      kind: "prototype",
  ...
  applied(session, {
    note: null,
    questions: [{ id: "q1", status: "reopened", answer: null }],
    visual: { drawing: null, queued: null },
  });
  let st = stateOf(session);
  ...
  applied(session, {
    visual: null,
    note: "The draw failed; click Visualize to try again.",
  });
  st = stateOf(session);
...
test("patch: terms are keyed by term; a known term is replaced whole, a new one appended", () => {
  const session = seeded({
    terms: [
      { term: "round", def: "One turn of questions.", avoid: ["batch"] },
...
test("patch: invalid JSON, a failed validation, a bad shape, or a missing state.json exits non-zero with one stderr line and leaves state.json untouched", () => {
  ...
  rejected(
    session,
    { questions: [{ id: "q1", status: "done" }] },
    /q1.*status/
  );
  rejected(
    session,
    { questions: [{ id: "q1", answer: { kind: "maybe" } }] },
    /q1.*answer/
  );
  rejected(
    session,
    { questions: [{ id: "q1", thread: [{ who: "bot", text: "hi" }] }] },
    /q1.*thread/
  );
  ...
  rejected(
    session,
    { visual: { stale: true } },
    /visual needs kind and version/
  );
  rejected(session, { terms: "round" }, /terms/);
  rejected(
    session,
    '{"questions":null,"__proto__":{"questions":[]}}',
    /__proto__.*the patch/
  );
  rejected(
    session,
    '{"agent":{"status":null,"__proto__":{"status":"waiting"}}}',
    /__proto__.*agent/
  );
  rejected(
    session,
    '{"questions":[{"id":"q1","status":null,"__proto__":{"status":"open"}}]}',
    /__proto__.*q1/
  );
  rejected(
    session,
    '{"questions":[{"id":"q9","round":2,"title":"t","rec":{"why":"w"},"__proto__":{"status":"answered"}}]}',
    /__proto__.*q9/
  );
...
test("patch: an error the merge never expected keeps the contract: one grill: line, exit 2, state.json untouched", () => {
  ...
  const r = rejected(
    session,
    deep,
    /could not apply the patch \(state\.json unchanged\)/
  );
...
test("patch: every field the page renders must have the shape the render reads, or the patch is rejected", () => {
  const session = seeded({
    ...
    questions: [
      qn("q1", 1, {
        explore: { at: T0, rows: [{ option: "A", pros: ["p"], cons: ["c"] }] },
      }),
    ],
    visual: {
      kind: "prototype",
  ...
  rejected(
    session,
    { terms: [{ term: "round", def: "d", avoid: "batch" }] },
    /round.*avoid/
  );
  rejected(
    session,
  ...
  const ex = (rows, at) => ({
    questions: [
      { id: "q1", explore: { ...(at === undefined ? {} : { at }), rows } },
    ],
  });
  ...
  rejected(
    session,
    ex([{ option: "A", pros: "fast", cons: ["c"] }]),
    /q1\.explore\.rows\[0\]\.pros/
  );
  rejected(
    session,
    ex([{ option: "A", pros: ["p"], cons: [{ text: "c" }] }]),
    /q1\.explore\.rows\[0\]\.cons/
  );
  rejected(
    session,
    ex([{ option: 1, pros: ["p"], cons: ["c"] }]),
    /q1\.explore\.rows\[0\]\.option/
  );
  ...
  rejected(
    session,
    { questions: [{ id: "q1", options: [{ k: "A", text: boom }] }] },
    /q1\.options/
  );
  rejected(
    session,
    { questions: [{ id: "q1", rec: { option: "A", why: boom } }] },
    /q1\.rec\.why/
  );
  rejected(
    session,
    { questions: [{ id: "q1", rec: { option: ["A"], why: "w" } }] },
    /q1\.rec\.option/
  );
  rejected(
    session,
    { questions: [{ id: "q1", rec: { text: 5, why: "w" } }] },
    /q1\.rec\.text/
  );
  rejected(
    session,
    {
      questions: [
        { id: "q1", status: "answered", answer: { kind: "option", option: 2 } },
      ],
    },
    /q1\.answer\.option/
  );
  rejected(
    ...
    {
      questions: [
        { id: "q1", status: "answered", answer: { kind: "text", text: boom } },
      ],
    },
    /q1\.answer\.text/
  );
  ...
  rejected(
    session,
    { visual: { drawing: { since: boom, seq: 2 } } },
    /visual\.drawing/
  );
  rejected(session, { finished: { doc: boom } }, /finished\.doc/);
  rejected(
    session,
    { finished: { doc: "docs/x.md", visual: 1 } },
    /finished\.visual/
  );
  rejected(
    session,
    { finished: { doc: "docs/x.md", at: boom } },
    /finished\.at/
  );
  ...
  applied(session, {
    terms: [
      { term: "round", def: "d", avoid: [] },
      { term: "send", def: "One press." },
    ],
    questions: [
      {
        id: "q1",
        status: "answered",
        answer: { kind: "text", text: "Neither, a third way" },
        rec: { text: "t", why: "w" },
        explore: {
          rows: [
            { option: "A", pros: ["p"], cons: [] },
            { option: "B", pros: [], cons: ["c"] },
          ],
        },
      },
      {
...
test("patch: a stdin that cannot be read exits non-zero with one stderr line and leaves state.json untouched", () => {
  ...
  let r;
  try {
    r = spawnSync(process.execPath, [SERVER, "patch", "--session", session], {
      env,
      stdio: [fd, "pipe", "pipe"],
      encoding: "utf8",
    });
...
test("patch: --file reads the patch from a file instead of stdin", () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      agent: { handled: 7 },
      questions: [{ id: "q1", status: "deferred" }],
    })
  );
  ...
  rejected(session, undefined, /no-such/, [
    "--file",
    join(tmp("grill-pf-"), "no-such.json"),
  ]);
});
...
test("patch round trip: new → patch round 1 → serve → POST /send → patch the handling → GET /state reflects it", async (t) => {
  ...
  const r1 = applied(session, {
    ...
    terms: [
      { term: "send", def: "One press of Send to Agent.", avoid: ["submit"] },
    ],
    questions: [
      {
        id: "q1",
        round: 1,
        title: "Storage",
        body: "Where state lives.",
        options: [
          { k: "A", text: "Files" },
          { k: "B", text: "SQLite" },
        ],
        rec: { option: "A", why: "No dependency." },
      },
      {
        id: "q2",
        round: 1,
        title: "Transport",
        body: "How the page talks.",
        options: [
          { k: "A", text: "Polling" },
          { k: "B", text: "SSE" },
        ],
        rec: { option: "A", why: "Simplest." },
      },
    ],
  });
  assert.deepEqual(JSON.parse(r1.out), {
    ok: true,
    questions: 2,
    open: 2,
    handled: 0,
  ...
  assert.deepEqual(
    st.questions.map((q) => [q.id, q.status]),
    [
      ["q1", "open"],
      ["q2", "open"],
    ]
  ...
  const actions = [
    { q: "q1", type: "answer", kind: "accept" },
    { q: "q2", type: "thread", text: "Why not SSE?" },
  ];
  assert.deepEqual(await (await post(s.ready.url, { actions })).json(), {
    ok: true,
    seq: 1,
  });
  const ev = JSON.parse(await s.out.nth(2));
  ...
  assert.ok(
    Date.parse(working.since) >= Date.parse(ev.at),
    `agent.since (${working.since}) stamped when work began, after the send (${ev.at})`
  );
  ...
  const r2 = applied(session, {
    ...
    questions: [
      { id: "q1", status: "answered", answer: { kind: "accept" } },
      {
        id: "q2",
        thread: [
          { who: "user", text: "Why not SSE?", at: ev.at },
          {
            who: "agent",
            text: "Polling survives a server restart with no reconnect logic.",
          },
        ],
      },
      {
        ...
        deps: ["q1"],
        title: "File layout",
        body: "With files settled in Q1.",
        options: [{ k: "A", text: "One folder" }],
        rec: { option: "A", why: "Easy to find." },
      },
    ],
  });
  ...
  assert.equal(st.agent.status, "waiting");
  assert.ok(
    Date.parse(st.agent.since) >= Date.parse(working.since),
    "agent.since restamped when waiting began"
  );
  assert.deepEqual(
    st.questions.map((q) => [q.id, q.status, q.round]),
    [
      ["q1", "answered", 1],
      ["q2", "open", 1],
      ["q3", "open", 2],
  ...
  assert.deepEqual(
    st.questions[1].thread.map((m) => m.who),
    ["user", "agent"]
  );
...
test("cli: an unknown subcommand prints usage and exits 2, including inherited names like toString", () => {
  for (const name of ["bogus", "toString", "__proto__", "constructor"]) {
    const r = spawnSync(process.execPath, [SERVER, name], {
      encoding: "utf8",
      env,
    });
